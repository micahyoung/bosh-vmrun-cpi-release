...
//go:generate counterfeiter -o fakes/fake_govc_client.go $GOPATH/src/bosh-vmrun-cpi/govc/govc.go GovcClient
...
//go:generate counterfeiter -o fakes/fake_govc_runner.go $GOPATH/src/bosh-vmrun-cpi/govc/govc.go GovcRunner
type GovcRunner interface {
	CliCommand(string, map[string]string, []string) (string, error)
}

//go:generate counterfeiter -o fakes/fake_govc_config.go $GOPATH/src/bosh-vmrun-cpi/govc/govc.go GovcConfig
